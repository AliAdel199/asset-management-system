...
export function TransferRequestsView() {
  ...
  const [message, setMessage] = useState("جاري تحميل طلبات النقل");
  const [isSubmitting, setIsSubmitting] = useState(false);

  function loadRequests() {
    return apiFetch(`/transfer-requests?status=${statusFilter}`).then(
      async (response) => {
        if (!response.ok) {
          throw new Error("تعذر تحميل طلبات النقل.");
        }
  ...
  useEffect(() => {
    ...
    loadRequests()
      ...
      .catch(() => {
        if (!ignore) {
          setMessage("تعذر تحميل طلبات النقل.");
        }
      });
  ...
  async function handleApprove(id: string) {
    ...
    try {
      const response = await apiFetch(`/transfer-requests/${id}/approve`, {
        method: "POST",
      });

      if (!response.ok) {
        const error = (await response.json()) as { message?: string };
        throw new Error(error.message ?? "تعذر اعتماد طلب النقل.");
      }

      await refresh();
      setMessage("تم اعتماد طلب النقل وتنفيذه.");
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : "تعذر اعتماد طلب النقل.",
      );
    } finally {
  ...
  async function handleAttach(event: FormEvent<HTMLFormElement>, id: string) {
    ...
    try {
      const response = await apiFetch(
        `/transfer-requests/${id}/attachments`,
  ...
  async function handleReject(event: FormEvent<HTMLFormElement>, id: string) {
    ...
    try {
      ...
      if (!response.ok) {
        const error = (await response.json()) as { message?: string };
        throw new Error(error.message ?? "تعذر رفض طلب النقل.");
      }
  ...
  return (
    <section className={styles.assetsWorkspace}>
      <div className={styles.sectionHeader}>
        <div>
          <p className={styles.eyebrow}>اعتماد مركزي</p>
          <h3>طلبات نقل الموجودات بين الجهات</h3>
        </div>
          ...
          <tbody>
            {requests.map((request) => (
              <tr key={request.id}>
                ...
                <td>{request.fromOrganizationUnit?.name ?? "غير محدد"}</td>
                <td>{request.toOrganizationUnit.name}</td>
                <td>{request.requestedByUser?.fullName ?? "غير معروف"}</td>
                <td>{formatDateTime(request.requestedAt)}</td>
                <td>{request.notes || request.documentNumber || "-"}</td>
            ...
            {requests.length === 0 && (
              <tr>
                <td colSpan={statusFilter === "PENDING" ? 7 : 6}>
                  لا توجد طلبات نقل ضمن هذا الفلتر.
                </td>
              </tr>
            )}
          </tbody>
